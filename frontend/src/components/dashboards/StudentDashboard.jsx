...
function MiniCalendar({ timetable }) {
  ...
  const classDows = new Set(
    timetable.map((t) => DAYS.indexOf(t.day) + 1).filter((n) => n > 0),
  );
        ...
        {cells.map((day, i) => {
          ...
          return (
            <div key={i} className="flex items-center justify-center">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-medium relative ${isToday ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}
              >
                {day}
                {hasClass && !isToday && (
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (r[0].status === "fulfilled")
        setTimetable(r[0].value?.timetables || []);
      if (r[1].status === "fulfilled")
        setNotices((r[1].value?.notices || []).slice(0, 4));
      if (r[2].status === "fulfilled")
        setAnnouncements((r[2].value?.announcements || []).slice(0, 4));
  ...
  const todayClasses = timetable
    .filter((t) => t.day === todayKey && t.slotType !== "BREAK")
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {[
            ...
            .map((tag, i) => (
              <span
                key={i}
                className="text-xs bg-white/15 text-indigo-100 px-3 py-1 rounded-full font-medium"
              >
                {tag}
              </span>
            ))}
        ...
        {[
          {
            ...
            warn: overall != null && overall < 75,
          },
          {
            l: "Today",
            v: todayClasses.length,
            icon: MdSchedule,
            color: "#6366f1",
          ...
          return (
            <Link
              key={c.l}
              to={c.to}
              className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 hover:border-slate-200 hover:shadow-md transition"
            >
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
            ...
            {loading ? (
              ...
              <div className="divide-y divide-slate-50">
                {todayClasses.map((cls, i) => {
                  ...
                  const status =
                    cur < start ? "upcoming" : cur <= end ? "ongoing" : "done";
                  const color = getColor(cls.title || cls.subject || "x");
                  return (
                    <div
                      key={cls._id || i}
                      className={`flex items-center gap-3 px-5 py-3.5 ${status === "done" ? "opacity-40" : ""}`}
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          ...
                          {cls.room ? ` · ${cls.room}` : ""}
                          {cls.faculty?.name ? ` · ${cls.faculty.name}` : ""}
                        </p>
                      </div>
                      {status === "ongoing" && (
                        <span className="shrink-0 text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-bold animate-pulse">
                          ● LIVE
                        </span>
                      )}
                      {status === "upcoming" && (
                        <span className="shrink-0 text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium">
                          Soon
                        </span>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <MdCampaign size={17} className="text-violet-500" />
                <h2 className="font-bold text-slate-900 text-sm">
                  Announcements
                </h2>
              </div>
            ...
            {loading ? (
              ...
              <div className="divide-y divide-slate-50">
                {announcements.map((a, i) => (
                  <div
                    key={a._id || i}
                    className="px-5 py-3.5 hover:bg-slate-50 transition"
                  >
                    <p className="font-semibold text-slate-900 text-sm">
                      {a.title}
                    </p>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <MdDescription size={17} className="text-amber-500" />
                <h2 className="font-bold text-slate-900 text-sm">
                  Latest Notices
                </h2>
              </div>
            ...
            {loading ? (
              <div className="p-6 flex justify-center">
                <div className="w-4 h-4 rounded-full border-2 border-slate-200 border-t-amber-500 animate-spin" />
              </div>
            ) : notices.length === 0 ? (
              <p className="p-6 text-center text-slate-400 text-sm">
                No notices
                ...
                {notices.map((n, i) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="text-[10px] bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full font-bold shrink-0">
                        {n.audience}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 mt-0.5 line-clamp-1">
              ...
              <div className="space-y-1.5">
                {DAYS.map((d) => {
                  const classes = timetable.filter(
                    (t) => t.day === d && t.slotType !== "BREAK",
                  );
                  const isToday = d === todayKey;
                  return (
                    <div
                      key={d}
                      className={`flex items-center gap-2 px-3 py-2 rounded-xl ${isToday ? "bg-indigo-50" : "bg-slate-50"}`}
                    >
                      <span
                        className={`text-[10px] font-extrabold w-8 ${isToday ? "text-indigo-600" : "text-slate-400"}`}
                      >
                        {d}
                      </span>
                      <div className="flex gap-1 flex-1 overflow-hidden">
                        {classes.slice(0, 5).map((c, i) => (
                          <div
                            ...
                            style={{
                              backgroundColor: getColor(c.title || "x"),
                            }}
                          />
                        ))}
                        {classes.length === 0 && (
                      ...
                      <span
                        className={`text-[10px] font-bold w-4 text-right ${isToday ? "text-indigo-600" : "text-slate-400"}`}
                      >
                        {classes.length}
                      </span>
                    </div>
