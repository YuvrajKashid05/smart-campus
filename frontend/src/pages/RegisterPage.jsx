...
import { MdArrowBack, MdArrowForward, MdSchool } from "react-icons/md";
...
const DEPTS = [
  ...
  { v: "IT", l: "Information Technology" },
  { v: "EE", l: "Electrical Eng." },
  { v: "ME", l: "Mechanical Eng." },
  { v: "CE", l: "Civil Eng." },
];
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.role === "STUDENT" && !form.rollNo.trim()) {
      setError("Roll number is required");
      return;
    }

    if (form.role === "STUDENT" && form.mobileNumber.trim().length < 10) {
      setError("Valid mobile number is required");
      return;
    }

    if (form.role === "FACULTY" && !form.employeeId.trim()) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="flex border-b border-slate-100">
            {["Account Info", "Profile Details"].map((label, index) => {
              const currentStep = index + 1;

              return (
                <div
                  key={currentStep}
                  className={`flex-1 flex items-center justify-center gap-2 py-4 text-xs font-semibold transition ${
                    step === currentStep
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  >
                    ...
                    {DEPTS.map((dept) => (
                      <option key={dept.v} value={dept.v}>
                        {dept.l}
                      </option>
                    ))}
                  </select>
