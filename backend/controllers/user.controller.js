...
export async function listUsers(req, res) {
  try {
    ...
    if (dept) {
      filter.dept = String(dept).trim().toUpperCase();
    }
...
export async function updateStudentByAdminOrFaculty(req, res) {
  try {
    ...
    if (student.role !== "STUDENT") {
      return res.status(400).json({
        ok: false,
        error: "Only student info can be updated from this route",
      });
    }

    const nextDept =
      parsed.data.dept !== undefined
        ? parsed.data.dept.trim().toUpperCase()
        : student.dept;

    const nextSection =
      parsed.data.section !== undefined
        ? parsed.data.section.trim().toUpperCase()
        : student.section;

    const nextRollNo =
      parsed.data.rollNo !== undefined
        ? parsed.data.rollNo.trim().toUpperCase()
        : student.rollNo;
    ...
    if (parsed.data.mobileNumber !== undefined) {
      student.mobileNumber = parsed.data.mobileNumber.trim();
    }
    if (parsed.data.dept !== undefined) {
      student.dept = parsed.data.dept.trim().toUpperCase();
    }
    if (parsed.data.section !== undefined) {
      student.section = parsed.data.section.trim().toUpperCase();
    }
    if (parsed.data.semester !== undefined) {
      student.semester = parsed.data.semester;
    }
    if (parsed.data.rollNo !== undefined) {
      student.rollNo = parsed.data.rollNo.trim().toUpperCase();
    }
    if (parsed.data.isActive !== undefined) {
      student.isActive = parsed.data.isActive;
    }
    ...
    if (err?.code === 11000) {
      return res
        .status(409)
        .json({ ok: false, error: "Duplicate student data found" });
    }
...
export async function deleteUserByAdmin(req, res) {
  try {
    ...
    if (user._id.toString() === req.user._id.toString()) {
      return res
        .status(400)
        .json({ ok: false, error: "Cannot delete your own account" });
    }
